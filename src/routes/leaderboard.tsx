...
function RankBadge({ rank }: { rank: number }) {
  ...
  return (
    <span className="flex h-8 w-8 items-center justify-center rounded-full bg-accent text-sm font-semibold text-muted-foreground">
      {rank}
    </span>
  );
}
...
function Avatar({ entry }: { entry: { full_name: string; username: string | null; avatar_url: string | null } }) {
  return (
    <div className="h-10 w-10 flex-shrink-0 overflow-hidden rounded-full bg-brand-tint">
      {entry.avatar_url ? (
        <img src={entry.avatar_url} alt={entry.full_name} className="h-full w-full object-cover" />
      ) : (
        <div className="flex h-full w-full items-center justify-center text-xs font-bold text-brand-hover">
      ...
      <div className="mx-auto max-w-6xl px-4 py-10 md:px-6 md:py-14">
        <div className="mb-8 text-center">
          <div className="mb-3 flex items-center justify-center gap-3">
            ...
            <h1 className="text-3xl font-bold text-foreground md:text-4xl">Tier List</h1>
          </div>
          <p className="text-sm text-muted-foreground">Rankings reset every month · {monthName}</p>
        </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <section className="rounded-2xl border border-border bg-background p-6">
              <div className="mb-5 flex items-start gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-foreground">Party Monster</h2>
                  <p className="text-sm text-muted-foreground">Most events attended this month</p>
                </div>
              </div>
              {partyMonster.length === 0 ? (
                <div className="rounded-xl bg-muted p-6 text-center">
                  <p className="text-sm text-muted-foreground">No rankings yet this month.</p>
                  <Link to="/events" className="mt-2 inline-block text-sm font-semibold text-primary hover:underline">
                ...
                <ol className="space-y-2">
                  {partyMonster.map((entry: PartyMonsterEntry) => (
                    <li key={entry.id} className="flex items-center gap-3 rounded-xl border border-accent p-3 transition hover:border-primary hover:bg-brand-tint/30">
                      ...
                      {entry.rank === 1 && (
                        <span className="rounded-full bg-primary px-2 py-0.5 text-xs font-bold text-white">#1</span>
                      )}
                    </li>
                  ))}
                </ol>
            ...
            <section className="rounded-2xl border border-border bg-background p-6">
              <div className="mb-5 flex items-start gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-foreground">Crowd Control</h2>
                  <p className="text-sm text-muted-foreground">Most group ticket purchases this month</p>
                </div>
              </div>
              {crowdControl.length === 0 ? (
                <div className="rounded-xl bg-muted p-6 text-center">
                  <p className="text-sm text-muted-foreground">No group buyers yet this month.</p>
                  <Link to="/events" className="mt-2 inline-block text-sm font-semibold text-primary hover:underline">
        ...
        <div className="mt-10 rounded-2xl border border-border bg-muted p-6 text-center">
          ...
          <Link to="/events" className="mt-3 inline-block rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white hover:bg-brand-hover">
            Discover Events
          </Link>
        </div>
      </div>
