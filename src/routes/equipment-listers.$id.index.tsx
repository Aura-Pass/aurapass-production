/**
 * Public equipment lister profile — gallery, videos, categories and active listings.
 */
import { useEffect, useState } from "react";
import { createFileRoute, Link } from "@tanstack/react-router";
import { Speaker, Play } from "lucide-react";
...
import { useEquipmentListerProfile } from "@/hooks/useEquipmentListers";
...
export const Route = createFileRoute("/equipment-listers/$id/")({
  head: () => ({
    meta: [
      { title: "Equipment Lister Profile | AuraPass" },
      {
        name: "description",
        content:
          "See photos, videos, equipment categories and active rental listings from this AuraPass equipment vendor.",
      },
      { property: "og:title", content: "Equipment Lister Profile | AuraPass" },
      {
        property: "og:description",
        content: "Photos, videos and active rental listings from this AuraPass equipment vendor.",
      },
      { property: "og:type", content: "profile" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: EquipmentListerProfilePage,
});

function EquipmentListerProfilePage() {
  const { id } = Route.useParams();
  const { lister, avatarUrl, listings, loading } = useEquipmentListerProfile(id);

  const [openIndex, setOpenIndex] = useState<number | null>(null);
  const [tiktokThumbs, setTiktokThumbs] = useState<Record<string, string>>({});

  const tiktokLinks = (lister?.video_links ?? []).filter(
    ...
    return (
      <PageWrapper>
        <div className="mx-auto max-w-3xl px-4 py-20 text-center md:px-6">
          <h1 className="text-2xl font-bold text-[#111827]">Equipment lister not found</h1>
          <p className="mt-2 text-sm text-[#6B7280]">This profile may not be approved yet.</p>
          <Button asChild variant="primary" size="sm" className="mt-6">
            <Link to="/equipment-listers">Back to equipment listers</Link>
          </Button>
        </div>
      </PageWrapper>
    );
  ...
  const photoUrls = lister.photo_urls ?? [];
  const videos = (lister.video_links ?? [])
    ...
    .filter(
      (
        ...
        raw: string;
        src: string;
        platform: NonNullable<ReturnType<typeof detectVideoPlatform>>;
  ...
  const mediaItems: MediaItem[] = [
    ...photoUrls.map((url) => ({
      kind: "image" as const,
      src: url,
      alt: `${lister.business_name} photo`,
    })),
    ...videos.map((v) => ({
      kind: "video" as const,
      src: v.platform === "youtube" ? `${v.src}?autoplay=1` : v.src,
      title: `${lister.business_name} video`,
      platform: v.platform,
    })),
  ];
    ...
    <PageWrapper>
      <div className="mx-auto max-w-5xl px-4 py-10 md:px-6">
          ...
          <div className="flex h-56 w-full shrink-0 items-center justify-center overflow-hidden rounded-xl bg-[#F3F4F6] md:h-56 md:w-56">
            {photoUrls[0] ? (
              ...
              >
                <img
                  src={photoUrls[0]}
                  alt={`${lister.business_name} equipment`}
        ...
        <section className="mt-10">
          <h2 className="text-xl font-semibold text-[#111827]">Listings</h2>
          {listings.length === 0 ? (
            <Card className="mt-4 p-8 text-center" style={{ borderRadius: 12 }}>
              <p className="text-[#6B7280]">No active listings from this vendor right now.</p>
            </Card>
          ) : (
              ...
              {listings.map((l) => (
                <Link key={l.id} to="/equipment/$id" params={{ id: l.id }} className="group block">
                  ...
                  >
                    ...
                    <div className="p-4">
                      <h3 className="text-lg font-semibold text-[#111827]">{l.title}</h3>
                      {l.category ? (
                        <div className="mt-2">
                  ...
                  >
                    {thumb ? (
                      <>
                        <img
                          src={thumb}
                          alt={`${lister.business_name} ${platformLabel[v.platform]} video thumbnail`}
                          loading="lazy"
