...
import { verifyBookingDeposit } from "@/lib/bookingPayments.functions";
...
export const Route = createFileRoute("/booking-payment-callback")({
  validateSearch: zodValidator(search),
  head: () => ({ meta: [{ title: "Confirming deposit — AuraPass" }] }),
  component: BookingPaymentCallbackPage,
});

function BookingPaymentCallbackPage() {
  const { reference, trxref } = Route.useSearch();
  const ref = reference || trxref;
  const navigate = useNavigate();
  const verify = useServerFn(verifyBookingDeposit);
